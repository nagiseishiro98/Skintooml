...
# ==========================
BOT_TOKEN = os.getenv("BOT_TOKEN", "8123122832:AAGXFFVTA_NBGv32iXXfUZV79QEqj46KM3s").strip()
...
def generate_unique_zip_name() -> str:
    """Generate a unique ZIP file name with timestamp and random string."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    random_str = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))
    return f"modified_pak_{timestamp}_{random_str}.zip"

def create_zip_with_pak(pak_file_path: Path, zip_filename: str) -> Path:
    """Create a ZIP file containing the PAK file."""
    zip_path = pak_file_path.parent / zip_filename
    
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        zipf.write(pak_file_path, arcname=pak_file_path.name)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "• ZIP packaging with unique names\n"
        "Commands:\n"
        "/unpack - Unpack First\n"
...
async def skin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Save modified PAK file
        pak_file_path = processor.work_dir / "game_patch_4.0.0.20349.pak"
        pak_file_path.write_bytes(modified_pak)

        # Generate unique ZIP name and create ZIP archive
        zip_filename = generate_unique_zip_name()
        zip_file_path = await loop.run_in_executor(None, create_zip_with_pak, pak_file_path, zip_filename)

        # Send ZIP file instead of PAK file
        with open(zip_file_path, "rb") as f:
            await context.bot.send_document(
                ...
                caption=(
                    f"🔥 Enhanced Skin Mod PAK (ZIP)\n\n"
                    f"✅ Applied: {modified_count} modifications\n"
                    f"📁 Files modified: {total_modified_files}\n"
                    f"📦 ZIP Size: {zip_file_path.stat().st_size / (1024*1024):.1f}MB\n"
                    f"📏 PAK Size: {len(modified_pak) / (1024*1024):.1f}MB\n\n"
                    f"Ready to use! 🚀"
                ),
            )
...
def main():
    ...
    print("✨ Features: Multi-patch, Null bytes validation, Advanced filtering, ZIP packaging")
